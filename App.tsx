...
export default function App() {
  ...
  const addExpense = async () => {
    ...
    try {
      ...
      setDesc('');
      setAmount('');
      setAiAnalysis(null); // ล้างวิเคราะห์เก่าเพื่อให้กดใหม่
    } catch (err) {
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50 no-print shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-2.5 bg-indigo-600 rounded-2xl text-white"><Wallet className="w-5 h-5" /></div>
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Cloud {loadingData ? 'Syncing...' : 'Connected'}</span>
              </div>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button onClick={loadUserData} disabled={loadingData} className={`p-2 rounded-full hover:bg-slate-100 transition-all ${loadingData ? 'animate-spin text-indigo-500' : 'text-slate-400'}`}>
              ...
              <div className="space-y-4">
                ...
                {loadingData ? (
                  <div className="flex flex-col items-center justify-center py-20 text-slate-300">
                    ...
                    <p className="font-bold text-sm">กำลังโหลดข้อมูลจาก Cloud...</p>
                  </div>
                ) : expenses.length === 0 ? (
